...
# from utils.hotmap import drawHotmap
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    callbacks.run('on_pretrain_routine_start') # 注册一个回调钩子，监控和干预模型的训练
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, irs, targets, rgb_paths, ir_paths, _) in pbar:  # batch -------------------------------------------------------------
            callbacks.run('on_train_batch_start')
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            # wangyu
            irs = irs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    irs = nn.functional.interpolate(irs, size=ns, mode='bilinear', align_corners=False)
            ...
            if RANK in {-1, 0}:
                ...
                callbacks.run('on_train_batch_end', model, ni, imgs, irs, targets, rgb_paths, ir_paths, list(mloss))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/cirdet.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/vedai.yaml', help='dataset.yaml path')
