...
class EFD(nn.Module):
    ...
    def forward(self, diff, x):
        ...
        # y = self.l1(y) # 融合各个通道的Feature map信息
        # y = self.relu(y)
        # y = self.l2(y) # 融合各个通道的Feature map信息
        # y = self.sig(y)
        # y = y.view(b, c, 1, 1)
        # _y = 1 - y # 得到与y互补的权重特征
        # # y.expand_as(x):将 y 扩展为与 x 具有相同的通道数，但在高度和宽度方向上保持不变
        # return x * y.expand_as(x), x * _y.expand_as(x)
...
class attention(nn.Module):
    ...
    def forward(self, x, x_e): #x 为主模态
        ...
        # short_cut = torch.cat([x,x_e],dim=3)  ####concat pool###### B H W 2C
...
if __name__ == '__main__':
    gelan_module = MyDecoup(256, 256)
    rgb = torch.randn(8, 256, 640, 640)
    ir = torch.randn(8, 256, 640, 640)
    input = [rgb, ir]
    output = gelan_module(input)
    # print(output.shape)